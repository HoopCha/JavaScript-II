...
ticketPriceTotal = runners.reduce((acc, currentValue) => {
...
// Problem 1: You need a list of emails along with their first name, to send a change of time to all the runners. 
let emails = [];
runners.forEach(object => {
  let emailname = object.email + " " + object.first_name;
  emails.push(emailname)
});
console.log(emails);
// Problem 2: People who donated over 200 dollars get a special bib, how many special bibs do you need?
let bigdonation = [];
bigdonation = runners.filter((runners) => {
  return runners.donation > 200;
});
console.log(bigdonation.length);
// Problem 3: you need a list of companies that donated, in alphabetical order

let companiesList = [];
companiesList = runners.map(runners => {
  return runners.company_name;
});
console.log(companiesList.sort());